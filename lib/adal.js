...
var AuthenticationContext = (function () {
     ...
     *  @property {string} cacheSetter - Custom function for cache writes
     *  @property {string} cacheGetter - Custom function for cache reads
     ...
     *  @property {Array.<string>} whitelistedOrigins If you want to whitelist certain additional origins for the popup messages
    ...
    AuthenticationContext = function (config) {
        ...
        if (!this.config.whitelistedOrigins) {
            this.config.whitelistedOrigins = [];
        }
    ...
    AuthenticationContext.prototype._loginPopup = function (urlNavigate, resource, callback) {
        var popupWindow = this._openPopup("/", "login", this.CONSTANTS.POPUP_WIDTH, this.CONSTANTS.POPUP_HEIGHT);
        ...
        //Standard does only work on same domain... in Confluence server we have cross-domain, so change this to message handling
        var handleMessage = function (messageEvent) {
            if (messageEvent.origin === location.origin || that.config.whitelistedOrigins.includes(messageEvent.origin)) {
                that.handleWindowCallback(messageEvent.data);
    ...
    */
    ...
    AuthenticationContext.prototype.acquireToken = function (resource, callback) {
        ...
        if (this._activeRenewals[resource]) {
            ...
            //Register handleCallback message
            var handleMessage = function (messageEvent) {
                if (messageEvent.origin === location.origin || that.config.whitelistedOrigins.includes(messageEvent.origin)) {
                    that.handleWindowCallback(messageEvent.data);
                    window.removeEventListener('message', handleMessage);
  ...
  */
    ...
    */
    ...
    AuthenticationContext.prototype._saveItem = function (key, obj, preserve) {
        if (this.config && this.config.cacheSetter)
            return this.config.cacheSetter(key, obj, preserve);
    ...
    AuthenticationContext.prototype._getItem = function (key) {
        if (this.config && this.config.cacheGetter)
            return this.config.cacheGetter(key);
